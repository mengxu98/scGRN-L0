...
#-------------------------------------------------------------------------------#
# Only for unfilter raw 10X files!
if (F) {
  ...
  # Manipulate data
  rownames(seu_obj_sce) <- uniquifyFeatureNames(rowData(seu_obj_sce)$ID, rowData(seu_obj_sce)$Symbol)
  colnames(seu_obj_sce) <- seu_obj_sce$Barcode
  my.counts <- counts(seu_obj_sce)
  ...
  set.seed(100)
  e.out <- emptyDrops(my.counts) # error: no counts available to estimate the ambient profile #because the object had filtered in cellranger
  ...
  names(is.cell) <- colnames(seu_obj_sce)
  seu_obj_sce$cells_kept <- is.cell

  seu_obj_sce <- seu_obj_sce[, seu_obj_sce$cells_kept == T]
  seu_obj_sce
}
