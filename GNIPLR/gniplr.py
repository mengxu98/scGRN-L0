...
ax_nl=0.04
n_d4='GNIPLR/BLCA_cancer.txt'
# train_s=pd.read_csv(+str(n_d4),sep='\t') # M
train_s=pd.read_csv(n_d4,sep='\t')
train_s.T.to_csv('GNIPLR/y1_10_d2_t.txt', sep='\t')
fw=open(n_d4[:-4]+'_gold.txt')
rels=[]
for p in fw:
	t1=p.split()
	rels.append(t1)
...
cancer1 = {}
samples1 = []
file1 = open("GNIPLR/y1_10_d2_t.txt")
for p in file1:
	fla+=1
	t1 = p.split()
	n1=len(t1)
...
fw=open('GNIPLR/e1_100_d2_p1.txt','w')
...
fw.close()
#############################
relust1=pd.read_csv('GNIPLR/e1_100_d2_p1.txt',sep='\t',names = range(4))
bbb=relust1.ix[:,[0,1,2,3]]
# bbb.to_csv(str(n_d4[:-4])+'_lasso.txt', sep='\t',index=False,header=False)
bbb.to_csv('GNIPLR/',n_d4[:-4]+'_lasso.txt', sep='\t',index=False,header=False)
