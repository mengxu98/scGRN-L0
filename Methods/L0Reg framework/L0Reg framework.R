    ...
    if (file.exists(paste('data/', target_gene, '_TFs_list.txt', sep = '')) == T) {
      ...
      if (peak_corr$p < 1 && peak_corr$r > 0) {
        ...
        if (nrow(res_data_f)>0) {
          ...
          res_TRN <- cbind("TF"=row.names(res_data_f),
                           "Reg"=res_data_f$reg,
                           "Gene"=target_gene,
          ...
          for (l in 1:nrow(res_data_f)) {
            ...
            # L0_gene <- X[gene,]*res_data_f$strength[l]
          ...
          nodes <- data.frame(name=c(target_gene,links$regulatoryGene),
          ...
          plot(network, 
               ...
               # edge.width=seq(1),edge.arrow.width=40,edge.arrow.size=4,
          ...
          # 添加图例
          legend("bottomleft", legend=levels(as.factor(V(network_L0)$carac)), 
          ...
          #L0 VS GENIE3
          Contrast <- data.frame(Cluster=paste0("Cluster",j),
                                 Gene = target_gene,
