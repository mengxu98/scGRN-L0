...
#' regulatorsList <- list(
#'   "Gene1" = rownames(exprMatrix)[1:10],
...
.GENIE3 <- function(exprMatrix, regulators, targets, treeMethod, K, nTrees, nCores, returnMatrix, verbose) {
  ...
  if (verbose) {
    message(paste("Tree method: ", treeMethod,
      "\nK: ", K,
  ...
  if (nCores == 1) {
    ...
    for (targetName in targetNames)
    {
      ...
      im <- .C(
        "BuildTreeEns", as.integer(num.samples), as.integer(numRegulators),
        ...
        as.double(vector("double", numRegulators))
      )[[12]]
