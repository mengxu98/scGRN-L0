...
### only merge the data sets of Ⅰ stage
...
# ggsave2("SuppFig2.pdf", path = "../results", width = 30, height = 30, units = "cm")

#SuppFig2 <- qc_std_plot(seu_obj_unfiltered)
#ggsave('../results/SuppFig2.png',SuppFig2, width = 30, height = 30, units = "cm")
...
# ggsave2("SuppFig3.pdf", path = "../results", width = 30, height = 30, units = "cm")
...
# ggsave2("SuppFig4.pdf", path = "../results", width = 30, height = 30, units = "cm")

DimPlot(seu_obj, group.by = "SCT_snn_res.0.2", label = T)
# ggsave2("SuppFig5.pdf", path = "../results", width = 30, height = 30, units = "cm")
...
for (i in seq_along(mainmarkers)) {
  ...
  # ggsave2(paste0("FeaturePlot_mainmarkers_", mainmarkers[i], ".pdf"), path = "../results", width = 10, height = 10, units = "cm")
}

DotPlot(seu_obj, features = mainmarkers, group.by = "SCT_snn_res.0.2") + 
  coord_flip() + 
  scale_color_viridis()
# ggsave2("DotPlot_mainmarkers.pdf", path = "../results", width = 30, height = 8, units = "cm")
...
annotation_curated_main <- read_excel("/data/mengxu/data/PhilipBischoff2021/curated_annotation/curated_annotation_main.xlsx")
...
ggsave2("Fig1.pdf", path = "../results", width = 15, height = 15, units = "cm")
#color scheme
use_colors <- c(
  p018 = "#E2D200",
  p019 = "#46ACC8",
...
ggsave2("Fig2.pdf", path = "../results", width = 15, height = 15, units = "cm")
#color scheme
use_colors <- c(
  Epithelial = "seagreen",
  Immune = "darkgoldenrod2",
...
ggsave2("Fig3_umap.pdf", path = "../results", width = 15, height = 15, units = "cm")
...
#   Plasma = "#0000CD")
# cell_types <- FetchData(seu_obj, vars = c("sample_id", "main_cell_type", "tissue_type")) %>% 
#   mutate(main_cell_type = factor(main_cell_type, levels = c("Plasma"))) %>% 
#   mutate(sample_id = factor(sample_id, levels = rev(c("p018t", "p019t", "p023t", "p024t", "p027t", "p028t",
#                                                       "p030t", "p031t", "p032t", "p033t", "p034t", "p018n", 
#                                                       "p019n", "p027n", "p028n", "p029n", "p030n", "p031n", 
#                                                       "p032n", "p033n", "p034n"))))
ggplot(data = cell_types) + 
  ...
  coord_flip()
ggsave2("Fig4_barplot.pdf", path = "../results", width = 15, height = 20, units = "cm")
...
#  umaptest <- RunUMAP(Plasma, dims = 1:i, verbose = F)
...
ggsave2("Fig5_epi.pdf", path = "../results", width = 30, height = 20, units = "cm")

DimPlot(imm, group.by = "SCT_snn_res.1", label = T, repel = T, split.by = "tissue_type")
ggsave2("Fig5_imm.pdf", path = "../results", width = 30, height = 20, units = "cm")

DimPlot(str, group.by = "SCT_snn_res.1", label = T, repel = T, split.by = "tissue_type")
ggsave2("Fig5_str.pdf", path = "../results", width = 30, height = 20, units = "cm")


### compare proportion of cells in a cluster to all epithelial cells for tumor and normal separately, 
### clusters overrepresented in normal samples are supposed to be cell of normal lung parenchyma, 
### all other clusters are supposed to be tumor cells
### 将一个簇中的细胞比例与肿瘤和正常的所有标准细胞分别进行比较，正常样本中过度表达的簇被认为是正常肺实质的细胞，所有其他簇被认为是肿瘤细胞
...
ggsave2("SuppFig6_epi_tumor_proportion.pdf", path = "../results", width = 30, height = 20, units = "cm")


### compare proportion of cells in a cluster to all immthelial cells for tumor and normal separately, 
### clusters overrepresented in normal samples are supposed to be cell of normal lung parenchyma, 
### all other clusters are supposed to be tumor cells

imm_clusters <- FetchData(imm, vars = c("SCT_snn_res.1", "tissue_type"))

count_tumor <- imm_clusters %>% filter(tissue_type == "Tumor") %>% count() %>% as.numeric()
count_normal <- imm_clusters %>% filter(tissue_type == "Normal") %>% count() %>% as.numeric()

imm_counts <- imm_clusters %>% group_by(tissue_type) %>% count(SCT_snn_res.1)
proportion_tumor <- imm_counts %>% filter(tissue_type == "Tumor") %>% mutate(proportion = n/count_tumor)
proportion_normal <- imm_counts %>% filter(tissue_type == "Normal") %>% mutate(proportion = n/count_normal)

proportion_imm <- full_join(proportion_normal, proportion_tumor, by = "SCT_snn_res.1") %>% 
  mutate(proportion.x = ifelse(is.na(proportion.x), 0,  proportion.x)) %>%  
  mutate(proportion.y = ifelse(is.na(proportion.y), 0,  proportion.y)) %>%
  mutate(tissue_type.x = "Normal") %>%
  mutate(tissue_type.y = "Tumor") %>%
...
cluster_type_data <- left_join(x = imm_clusters, y = proportion_imm, by = "SCT_snn_res.1")
rownames(cluster_type_data) <- rownames(imm_clusters)

imm<- AddMetaData(imm, select(cluster_type_data, cluster_type))


### Bar plot for figure 2

n1 <- select(proportion_imm, c(tissue_type.x, SCT_snn_res.1, proportion.x)) %>%
  mutate(tissue_type = tissue_type.x) %>% 
  mutate(proportion = proportion.x) %>%
  mutate(tissue_type.x = NULL) %>%
  mutate(proportion.x = NULL)
t1 <- select(proportion_imm, c(tissue_type.y, SCT_snn_res.1, proportion.y)) %>%
  mutate(tissue_type = tissue_type.y) %>% 
  mutate(proportion = proportion.y) %>%
  mutate(tissue_type.y = NULL) %>%
  mutate(proportion.y = NULL)

proportion_imm2 <- rbind(n1, t1)
use_colors <- c(
  Tumor = "brown2",
  Normal = "deepskyblue2")
ggplot(proportion_imm2, aes(fill = tissue_type, y = proportion, x = SCT_snn_res.1)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  scale_fill_manual(values = use_colors)

ggsave2("SuppFig6_imm_tumor_proportion.pdf", path = "../results", width = 30, height = 20, units = "cm")

### compare proportion of cells in a cluster to all strthelial cells for tumor and normal separately, 
### clusters overrepresented in normal samples are supposed to be cell of normal lung parenchyma, 
### all other clusters are supposed to be tumor cells

str_clusters <- FetchData(str, vars = c("SCT_snn_res.1", "tissue_type"))

count_tumor <- str_clusters %>% filter(tissue_type == "Tumor") %>% count() %>% as.numeric()
count_normal <- str_clusters %>% filter(tissue_type == "Normal") %>% count() %>% as.numeric()

str_counts <- str_clusters %>% group_by(tissue_type) %>% count(SCT_snn_res.1)
proportion_tumor <- str_counts %>% filter(tissue_type == "Tumor") %>% mutate(proportion = n/count_tumor)
proportion_normal <- str_counts %>% filter(tissue_type == "Normal") %>% mutate(proportion = n/count_normal)

proportion_str <- full_join(proportion_normal, proportion_tumor, by = "SCT_snn_res.1") %>% 
  mutate(proportion.x = ifelse(is.na(proportion.x), 0,  proportion.x)) %>%  
  mutate(proportion.y = ifelse(is.na(proportion.y), 0,  proportion.y)) %>%
  mutate(tissue_type.x = "Normal") %>%
  mutate(tissue_type.y = "Tumor") %>%
...
cluster_type_data <- left_join(x = str_clusters, y = proportion_str, by = "SCT_snn_res.1")
rownames(cluster_type_data) <- rownames(str_clusters)

str<- AddMetaData(str, select(cluster_type_data, cluster_type))


### Bar plot for figure 2

n1 <- select(proportion_str, c(tissue_type.x, SCT_snn_res.1, proportion.x)) %>%
  mutate(tissue_type = tissue_type.x) %>% 
  mutate(proportion = proportion.x) %>%
  mutate(tissue_type.x = NULL) %>%
  mutate(proportion.x = NULL)
t1 <- select(proportion_str, c(tissue_type.y, SCT_snn_res.1, proportion.y)) %>%
  mutate(tissue_type = tissue_type.y) %>% 
  mutate(proportion = proportion.y) %>%
  mutate(tissue_type.y = NULL) %>%
  mutate(proportion.y = NULL)

proportion_str2 <- rbind(n1, t1)
use_colors <- c(
  Tumor = "brown2",
  Normal = "deepskyblue2")
ggplot(proportion_str2, aes(fill = tissue_type, y = proportion, x = SCT_snn_res.1)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  scale_fill_manual(values = use_colors)

ggsave2("SuppFig6_str_tumor_proportion.pdf", path = "../results", width = 30, height = 20, units = "cm")
...
tcell_nk_markers <- c("CD3E", "CD4", "FOXP3", "IL7R", "IL2RA", "CD40LG", "CD8A", "CCL5", "NCR1", "NKG7", "GNLY", 
                      "NCAM1", "KLRD1", "KLRB1", "CD69", "KLRG1", "MKI67", "CDK1", "EPCAM")

bcell_plasma_mast_markers <- c("MS4A1", "CD19", "CD79A", "JCHAIN", "IGHA1", "IGHA2", "IGHG1", "IGHG2", "IGHG3", 
                               "IGHG4", "IGKC", "IGLC2", "IGLC3", "CPA3", "KIT", "MS4A2", "GATA2",  "MKI67", "CDK1", "EPCAM")

DotPlot(imm, features = myeloid_markers, group.by = "SCT_snn_res.0.5") + coord_flip()
#ggsave2("DotPlot_myeloid_markers.pdf", path = "../results", width = 20, height = 20, units = "cm")

DotPlot(imm, features = tcell_nk_markers, group.by = "SCT_snn_res.0.5") + coord_flip()
#ggsave2("DotPlot_T_NK_markers.pdf", path = "../results", width = 20, height = 20, units = "cm")

DotPlot(imm, features = bcell_plasma_mast_markers, group.by = "SCT_snn_res.0.5") + coord_flip()
#ggsave2("DotPlot_B_Plasma_markers.pdf", path = "../results", width = 20, height = 20, units = "cm")


### other papers
# plasma B-cells markgenes
# Intratumoral plasma cells predict outcomes to PD-L1 blockade in non-small cell lung cancer
# "MZB1", "DERL3", "JSRP1", "TNFRSF17", "SLAMF7", "IGHG2","IGHGP", "IGLV3-1", "IGLV6-57", "IGHA2", "IGKV4-1","IGLC7", "IGLL5"
# not fount genes: IGKV1-12

# Phenotype molding of stromal cells in the lung  tumor microenvironment
# IGHG4, IGHG3, IGHG1, IGHGP, IGHG2

# plasmacytoid dendritic cells markgenes
# GZMB, LILRA4, CLIC3, 
# SMPD3, PLD4,  LILRB4, IRF7, IL3RA, CXCR3, MAP1A,  PLAC8, 
# PTCRA, JAML, UGCG, LAMP5,  SCT, PPP1R14B, IRF4, SEC61B, 
# ITM2C,  C9orf142, CLN8, RNASE6, RASD1, IRF8,  SPIB, TCL1A, GPR183
# not fount genes: C9orf142
plasma_markers <- c("MZB1", "DERL3", "JSRP1", "TNFRSF17", "SLAMF7", "IGHG2","IGHG1","IGHG4", "IGHGP", "IGLV3-1", 
                    "IGLV6-57", "IGHA2", "IGKV4-1", "IGLC7", "IGLL5")

plasma_cytoid_dendritic_markers <- c("GZMB", "LILRA4", "CLIC3", "SMPD3", "PLD4",  "LILRB4", "IRF7", "IL3RA", "CXCR3", 
"MAP1A",  "PLAC8", "PTCRA", "JAML", "UGCG", "LAMP5",  "SCT", "PPP1R14B", "IRF4", "SEC61B", "ITM2C", "CLN8", 
"RNASE6", "RASD1", "IRF8",  "SPIB", "TCL1A", "GPR183")

DotPlot(imm, features = plasma_markers, group.by = "SCT_snn_res.0.2") + coord_flip()
ggsave2("DotPlot_plasma_markers.pdf", path = "../results", width = 20, height = 20, units = "cm")

DotPlot(imm, features = plasma_cytoid_dendritic_markers, group.by = "SCT_snn_res.0.2") + coord_flip()
ggsave2("DotPlot_plasma_cytoid_dendritic_markers.pdf", path = "../results", width = 20, height = 20, units = "cm")


###

DimPlot(imm, group.by = "SCT_snn_res.0.2", label = T, split.by = "tissue_type")
ggsave2("DimPlot_immune_clusters.pdf", path = "../results", width = 25, height = 15, units = "cm")

DimPlot(epi, group.by = "SCT_snn_res.0.2", label = T, split.by = "tissue_type")
ggsave2("DimPlot_epi_clusters.pdf", path = "../results", width = 25, height = 15, units = "cm")

DimPlot(str, group.by = "SCT_snn_res.0.5", label = T, split.by = "tissue_type")
ggsave2("DimPlot_str_clusters.pdf", path = "../results", width = 25, height = 15, units = "cm")
...
ggsave2("DimPlot_immune_patients.pdf", path = "../results", width = 25, height = 15, units = "cm")

DimPlot(epi, group.by = "patient_id", split.by = "tissue_type", cols = use_colors)
ggsave2("DimPlot_epi_patients.pdf", path = "../results", width = 25, height = 15, units = "cm")

DimPlot(str, group.by = "patient_id", split.by = "tissue_type", cols = use_colors)
ggsave2("DimPlot_str_patients.pdf", path = "../results", width = 25, height = 15, units = "cm")
...
for (i in seq_along(sheets)) {
  a <- read_excel("/data/mengxu/data/PhilipBischoff2021/data/media-3.xlsx", sheet = sheets[[i]])
...
teichmann_signatures_epi <- read.csv("/data/mengxu/data/PhilipBischoff2021/data/Fig1_DE_Lung_atlas_epithelial.csv")
...
teichmann_signatures_imm <- read.csv("/data/mengxu/data/PhilipBischoff2021/data/Fig2_DE_Lung_atlas_immune.csv")
...
annotation_curated_epi <- read_excel("/data/mengxu/data/PhilipBischoff2021/curated_annotation/curated_annotation_epi.xlsx")
epi_anno <- epi
...
annotation_curated_imm <- read_excel("/data/mengxu/data/PhilipBischoff2021/curated_annotation/curated_annotation_imm.xlsx")
imm_anno <- imm
...
annotation_curated_str <- read_excel("/data/mengxu/data/PhilipBischoff2021/curated_annotation/curated_annotation_str.xlsx")
str_anno <- str
...
imm_type %>%
  ...
  scale_fill_gradientn(colors = c("blue", "white", "red"),
                       ...
                       labels = c("0", "1")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), axis.text.y = element_text(hjust = 1))
ggsave2("SuppFig7.pdf", path = "../results", width = 30, height = 30, units = "cm")
...
broad_pws <- read_lines("/data/mengxu/data/PhilipBischoff2021/data/h.all.v6.2.symbols.gmt") %>%
  lapply(str_split, "\\t") %>% 
  unlist(recursive = F) %>% 
...
scna_scores <- read.delim("/data/mengxu/data/PhilipBischoff2021/inferCNV_output/infercnv_clone_scores_nsclc.tsv") %>% filter(tissue_type == "Tumor") %>% filter(!is.na(cna_clone))
...
ggsave2("Fig2B.pdf", path = "../results", width = 11, height = 8, units = "cm")


DimPlot(epi_anno, group.by = "cell_type_epi", cols = use_colors, pt.size = 0.5)
ggsave2("Fig2A_celltype.pdf", path = "../results", width = 15, height = 15, units = "cm")

DimPlot(epi_anno, group.by = "patient_id", cols = use_colors, pt.size = 0.5)
ggsave2("Fig2A_patients.pdf", path = "../results", width = 15, height = 15, units = "cm")

DimPlot(epi_anno, group.by = "tissue_type", cols = use_colors, pt.size = 0.5)
ggsave2("Fig2A_tissuetype.pdf", path = "../results", width = 15, height = 15, units = "cm")
...
ggsave2("SuppFig4_umap_cna_clone.pdf", path = "../results", width = 15, height = 15, units = "cm")

DimPlot(epi_anno, group.by = "cluster_type", cols = c("cyan3", "darkorange2"), pt.size = 0.5)
ggsave2("SuppFig4_umap_cluster_type.pdf", path = "../results", width = 15, height = 15, units = "cm")
...
ggsave2("Fig2C.pdf", path = "../results", width = 30, height = 30, units = "cm")
...
ggsave2("SuppFig3F.pdf", path = "../results", width = 10, height = 20, units = "cm")
...
for (i in c(4, 6, 8)){
  ...
  ggsave2(paste0("SuppFig3E_", i, "dimensions_patient.pdf"), path = "../results", width = 15, height = 10, units = "cm")
  print(DimPlot(umaptest, reduction = "umap", group.by = "histo_subtype", cols = use_colors) + labs(title = paste0(i, " dimensions")) + coord_fixed())
  ggsave2(paste0("SuppFig3E_", i, "dimensions_histo.pdf"), path = "../results", width = 15, height = 10, units = "cm")
  print(FeaturePlot(umaptest, features = "PC_1", order = T, cols = viridis(10)) + labs(title = paste0(i, " dimensions")) + coord_fixed())
  ggsave2(paste0("SuppFig3E_", i, "dimensions_PC1.pdf"), path = "../results", width = 15, height = 10, units = "cm")
  remove(umaptest)
}
...
DoMultiBarHeatmap(str_endo, features = top_endo_markers$gene, group.by = "cell_type_str", additional.group.by = "tissue_type",additional.group.sort.by = "tissue_type", cols.use = list(tissue_type = use_colors), draw.lines = F) +
  scale_fill_viridis()
ggsave2("SuppFig6B.pdf", path = "../results", width = 30, height = 40, units = "cm")
...
DoMultiBarHeatmap(str_fibro, features = top_fibro_markers$gene, group.by = "cell_type_str", additional.group.by = "tissue_type",additional.group.sort.by = "tissue_type", cols.use = list(tissue_type = use_colors), draw.lines = F) +
  scale_fill_viridis()
ggsave2("Fig6C.pdf", path = "../results", width = 30, height = 40, units = "cm")
...
#ggsave2("DimPlot_imm_Normal_Tumor.pdf", path = "output/fig4", width = 35, height = 15, units = "cm")
...
ggsave2("SuppFig1C_imm_patients.pdf", path = "../results", width = 15, height = 15, units = "cm")
...
ggsave2("Fig4A_umap.pdf", path = "../results", width = 35, height = 15, units = "cm")
...
ggsave2("SuppFig7B.pdf", path = "../results", width = 30, height = 40, units = "cm")
...
ggsave2("SuppFig7C.pdf", path = "../results", width = 30, height = 35, units = "cm")
...
m1m2_pws <- read_lines("/data/mengxu/data/PhilipBischoff2021/data/CLASSICAL_M1_VS_ALTERNATIVE_M2_MACROPHAGE_UP.gmt") %>%
  lapply(str_split, "\\t") %>% 
  unlist(recursive = F) %>% 
  lapply(function(x) setNames(list(x[-c(1:2)]), x[1])) %>% 
  unlist(recursive = F)

m1m2_pws <- append(m1m2_pws, read_lines("/data/mengxu/data/PhilipBischoff2021/data/CLASSICAL_M1_VS_ALTERNATIVE_M2_MACROPHAGE_DN.gmt") %>%
                     lapply(str_split, "\\t") %>% 
                     unlist(recursive = F) %>% 
                     lapply(function(x) setNames(list(x[-c(1:2)]), x[1])) %>% 
                     unlist(recursive = F))
...
T_exhausted <- read_excel("/data/mengxu/data/PhilipBischoff2021/data/CD8_T_cells_exhausted.xlsx", skip = 1)
...
png("../results/SuppFig8A.pdf", width = 30, height = 30, units = "cm", res = 600)
...
CD123 <- read_excel("/data/mengxu/data/PhilipBischoff2021/IHC_quantification/Quantification_CD123.xlsx")
...
CTHRC1 <- read_excel("/data/mengxu/data/PhilipBischoff2021/IHC_quantification/Quantification_CTHRC1.xlsx")
...
CXCL9 <- read_excel("/data/mengxu/data/PhilipBischoff2021/IHC_quantification/Quantification_CXCL9.xlsx")
...
luad_se <- loadHDF5SummarizedExperiment(dir="/data/mengxu/data/PhilipBischoff2021/TCGA LUAD")
luad_mut <- read_csv("/data/mengxu/data/PhilipBischoff2021/data/TCGA_LUAD_mutations.csv")
...
png(file = "../results/SuppFig8B.pdf", width = 30, height = 30, units = "cm", res = 600)
