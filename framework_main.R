...
NIMEFI <- function(expressionMatrix, predictorIndices = NULL, targetIndices = NULL, GENIE = TRUE, SVM = TRUE, EL = TRUE,
    ...
    if (is.null(outputFileName)) {
        message("----- Please set file path! -----")
        return(resultMatrix)
    } else {
        if (is.null(outputFileFormat)) {
            message("----- Please set file format! -----")
        } else {
            if (outputFileFormat == "txt") {
                write.table(resultMatrix, paste0(outputFileName, ".", outputFileFormat), row.names = FALSE, col.names = FALSE, sep = "\t", quote = FALSE)
            } else if (outputFileFormat == "csv") {
                write.table(resultMatrix, paste0(outputFileName, ".", outputFileFormat), row.names = FALSE, col.names = FALSE, sep = ",", quote = FALSE)
            }
        }
    }
